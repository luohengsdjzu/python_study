...
# print(d_iterator.__next__())     # 返回一个值’a‘
# print(d_iterator.__next__())      # 返回一个值’b‘
# print(d_iterator.__next__())      # 返回一个值’c‘
...
#       迭代器对象.__iter__():得到迭代器的本身，说白了调了跟没调一个样（为了统一for循环）

# dic = {'a':1, 'b':2, 'c':3}
# dic_iterator = dic.__iter__()
#
# print(dic_iterator is dic_iterator.__iter__() is dic_iterator.__iter__().__iter__())
...
# 3、循环步骤2，知道抛出StopIterator异常，for循环捕捉异常然后退出循环
for k in d:
    print(k)
...
#       而对于其他容器类型，如列表，需要把所有的元素都存放于内存中，受内存大小的限制，
#       可以存放的值的个数是有限的。
# 缺点：1、除非取尽，否则无法获取迭代器的长度
#       2、只能取下一个值，不能回到开始，更像是‘一次性的’，
#       迭代器产生后的唯一目标就是重复执行next方法直到值取尽，否则就会停留在某个位置，
#       等待下一次调用next；若是要再次迭代同个对象，
#       你只能重新调用iter方法去创建一个新的迭代器对象，
