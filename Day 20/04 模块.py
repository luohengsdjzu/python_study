...
1、什么是模块？
    模块就是一系列功能的集合，分为三大类：
        I：内置的模块
        II：第三方的模块
        III：自定义的模块(可以是python、c、c++等语言编写的模块)
...
2、为何要用模块？
    I：内置与第三方的模块拿来就用，无需定义，这种拿来主义，可以极大地提升开发效率
    II：自定义的模块
...
# import foo
# 首次导入模块发生三件事：
# 1、产生foo.py的名称空间，将foo.py运行过程中产生的名字都丢到foo的名称空间
...
# 强调1：模块名.名字，是指名道姓地问某一个模块要名字对应的值，不会与当前名称空间中的名字发生冲突下·
